...
module AwsSdb
  ...
  class Service
    ...
    def select(params)
      ...
      query = "select #{attributes} from #{domain} where #{where}"
      query += " limit #{limit}" unless limit.nil? || limit.to_i == 0
      ...
      params['NextToken'] =
        next_token unless next_token.nil? || next_token.empty?

      doc = call(:get, params)

      results = []

      unless attributes == "count(*)"
...
#          results << item.to_s
        ...
        REXML::XPath.each(doc, '//Item') do |item|
          hash = {}
          hash[:itemName] = item.elements["Name"].text

          item.elements.each("Attribute") do |attribute|
            hash[attribute.elements["Name"].text] =
              attribute.elements["Value"].text
          end
          results << hash

        end
      else
        REXML::XPath.each(doc, "//Item/Attribute/Value/text()") do |item|
          results << item.to_s
        end
      end
      return results, REXML::XPath.first(doc, '//NextToken/text()').to_s
